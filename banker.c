...
int m, n, i, j, al[10][10], max[10][10], av[10], need[10][10], temp, z, y, p, k,flag=0;
void main() {
...
printf("Enter allocation matrix: \n");
  ...
  while (p != 0) {
    ...
    for (i = 0; i < m; i++) {
      z = 0;
      for (j = 0; j < n; j++) {
        if (need[i][j] <= av[j] && (need[i][0] != -1)) { // comparing need with available instance and
